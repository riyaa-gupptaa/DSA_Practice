...
int f(int i,int j,vector<vector<int>> &matrix,vector<vector<int>> &dp)
{
    ...
    return dp[i][j]=max(straight, max(left, rd));
}
int maxFallingPathSum(vector<vector<int>> &matrix)
{
    ...
    for(int j=0;j<m;j++)
    {
        result=max(result,f(n-1,j,matrix,dp));
    }
    return result;
}
...
int maxFallingPathSum(vector<vector<int>> &matrix)
{
    ...
    for(int i=1;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            ...
            dp[i][j]=matrix[i][j]+max(straight,max(left,rd));
        }
    }
    int result=INT_MAX;
    for(int j=0;j<m;j++)
...
int maxFallingPathSum(vector<vector<int>> &matrix)
{
    ...
    for(int i=n-2;i>=0;--i)
    {
        ...
        {
            int down=prev[j];
            int ld=(j>0)?prev[j-1]:INT_MIN;
            int rd=(j<n-1)?prev[j+1]:INT_MIN;
            curr[j] = matrix[i][j] + max(down, max(ld,rd));
        }
        prev = curr;
    }
